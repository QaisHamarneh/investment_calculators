...
def retirement_calculator(expected_monthly_salary: float,
    ...
    inflated_salary = inflation_calculator.inflation_calculator(yearly_salary, inflation_rate, remaining_period)[-1]
    required_investment = inflated_salary * 100 / (interest_rate - inflation_rate - breathing_room)

    low, high = 0, 100
    percentage_deposit = low + (high - low) / 2
    _, _, accumulated_investments = accumulated_interest_calculator.interest_calculator(starting_amount,
                                                                                        interest_rate,
                                                                                        remaining_period,
